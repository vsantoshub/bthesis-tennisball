//
// OCVCapture_MJPEG - Code for capturing video frames using
...
OCVCapture_MJPEG::OCVCapture_MJPEG()
{
...
void OCVCapture_MJPEG::setVerbose(bool verboseOn)
{
	m_verbose = verboseOn;
}

bool OCVCapture_MJPEG::verbose() const
{
	return m_verbose;
}

void OCVCapture_MJPEG::setDesiredSize(uint32_t width, uint32_t height)
{
	if (isOpen())
	{
...
void OCVCapture_MJPEG::setFramerate(uint32_t fps)
{
    if (isOpen())
    {
        reportError("cannot change framerate while open");
        return;
    }
...
void OCVCapture_MJPEG::setPixelFormat(char* pixfmt)
{
    if (isOpen())
    {
        reportError("cannot change pixel format while open");
        return;
    }

    if (strcmp(pixfmt,"YUYV") == 0){
        m_desired_pix_fmt = V4L2_PIX_FMT_YUYV;
        cout << messageHeader << "pix_fmt YUYV" << endl;
    } else if (strcmp(pixfmt,"MJPEG") == 0){
        m_desired_pix_fmt = V4L2_PIX_FMT_MJPEG;
        cout << messageHeader << "pix_fmt MJPEG" << endl;
    } else if (strcmp(pixfmt,"H264") == 0){
        m_desired_pix_fmt = V4L2_PIX_FMT_H264;
        cout << messageHeader << "pix_fmt H264" << endl;
    } else {
        reportError("Sorry, I don't handle that pixel format");
        return;
    }

}

void OCVCapture_MJPEG::deviceID(char* id)
{
    if (isOpen())
    {
...
void OCVCapture_MJPEG::reportError(const char *error)
{
	cerr << messageHeader << error << endl;
}

void OCVCapture_MJPEG::reportError(const char* error, int64_t value)
{
	cerr << messageHeader << error << " " << value << endl;
}

int OCVCapture_MJPEG::retry_ioctl(int request, void* argument)
{
	int result;
...
bool OCVCapture_MJPEG::isOpen() const
{
	return (m_camera_handle > 0);
}

uint32_t OCVCapture_MJPEG::width() const
{
	return m_final_width;
}

uint32_t OCVCapture_MJPEG::height() const
{
	return m_final_height;
}

uint32_t OCVCapture_MJPEG::frameRate() const
{
	return m_final_frame_rate;
}

bool OCVCapture_MJPEG::open()
{
	if (isOpen())
		return true;
	...
	if (m_verbose)
	{
		...
		cout << messageHeader << "bytes per line " << m_raw_bytes_per_line << endl;
	}
	...
	//} else {
    //    cout << messageHeader << "camera does not support timeperframe" << endl;
	//}
	...
	if (getfps.parm.capture.capability & V4L2_CAP_TIMEPERFRAME)
        cout << messageHeader << "camera supports timeperframe" << endl;
	...
	m_raw_image_size = m_mapped_buffer_lens[0];//m_final_height * m_raw_bytes_per_line;
	dataBuffer.resize(m_raw_image_size);
	m_raw_image_data = &dataBuffer[0];
	//m_raw_image_data = new (nothrow) uint8_t[m_raw_image_size];
...
bool OCVCapture_MJPEG::firstGrabSetup()
{
	if (m_first_grab)
	{
...
void OCVCapture_MJPEG::resizeMat(Mat& mat, int matType)
{
	if (mat.empty() ||
		(unsigned int) mat.rows != m_final_height ||
		(unsigned int) mat.cols != m_final_width ||
		mat.type() != matType)
	{
...
bool OCVCapture_MJPEG::yuv2gray(Mat& grayMat)
{
	if (!isOpen())
		return false;
	...
	{
		const uint8_t *getIt = m_raw_image_data + rowIndex * m_raw_bytes_per_line;
...
bool OCVCapture_MJPEG::yuv2rgb(Mat& rgbMat)
{
	if (!isOpen())
		return false;
...
bool OCVCapture_MJPEG::yuv2yuv(Mat& yuvMat)
{
	if (!isOpen())
		return false;
...
bool OCVCapture_MJPEG::mjpeg2rgb(Mat& rgbMat)
{
    if (!isOpen())
        return false;
    ...
    rgbMat = imdecode(dataBuffer,CV_LOAD_IMAGE_COLOR);
    return true;
}

bool OCVCapture_MJPEG::rgb(Mat& rgbMat)
{
    switch (m_desired_pix_fmt){
    case V4L2_PIX_FMT_YUYV:
        return OCVCapture_MJPEG::yuv2rgb(rgbMat);
        break;
    case V4L2_PIX_FMT_MJPEG:
        return OCVCapture_MJPEG::mjpeg2rgb(rgbMat);
        break;
    default:
